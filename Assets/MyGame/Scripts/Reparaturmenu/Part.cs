...
public class Part : MonoBehaviour {
    ...
    void OnMouseDrag()
    {
        ...
        ray = new Ray(this.transform.position*1.1f, this.transform.position - Camera.main.transform.position);
		RaycastHit hit;
		if (Physics.Raycast(ray, out hit, 9999))
        {
			...
			Debug.Log($"hitObject Ref: {hitObject}");
            Debug.Log($"windmill Ref: {mb}");
            if(mb != null && mb.brokenL > 0){
                mb.repair();
                Debug.Log("repairing windmill");
            }
